...
- **Segurança e Integridade** - Sistema de backups rotativos, validação por checksum SHA-256 e mecanismo de _Flush_ no encerramento para evitar corrupção de dados.

- [x] **Ability System (v0.1.0)** - Framework de Gameplay Orientado a Dados

Uma solução madura e nativa de alto desempenho (GDExtension/Module) projetada para mecânicas de jogo complexas e arquiteturas de produção.

**Orquestração Core:**

- `ASComponent` - O cérebro determinístico (exclusivo physics) que gerencia atributos e Specs ativos.
- `ASContainer` - Template de arquétipo de classe para inicialização rápida de entidades (ex: "Mago", "Guerreiro").
- `ASDelivery` & `ASPackage` - Injetores de lógica agnósticos que desacoplam o gameplay da infraestrutura de colisão.
- `ASAttributeSet` & `ASAttribute` - Sistema de stats dinâmicos com **Attribute Drivers** para cálculo reflexivo de valores.

**Inteligência de Runtime (Specs & Tags):**

- **Soberania dos Specs** - `ASAbilitySpec`, `ASEffectSpec` e `ASCueSpec` gerenciam o estado de execução, permitindo o compartilhamento de recursos entre múltiplos atores.
- **Motor de Tags Globais** - Mapeamento hierárquico (`AbilitySystem) Singleton) com regras lógicas nativas (`Required`, `Blocked`) e **Ability Triggers** automatizados.
- **Ponte Nativa LimboAI** - Nós de Behavior Tree sincronizados para controle de IA autoritativa.

**Multiplayer e Determinismo:**

- `ASStateCache` - Buffer de histórico leve para predição local otimizada.
- `ASStateSnapshot` - Captura total de estado para Save/Load persistente e Rollback pelo servidor.
- **Padrões de API Seguros** - Uso mandatório de `try_activate`, `can_` e `request_` (RPC) para prevenção de mutações indevidas.
